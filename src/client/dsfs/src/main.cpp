...
int main(int argc, char *argv[]) {
  ...
  encfs_oper.open = dsfs::operations::open;
  // encfs_oper.read = encfs_read;
  encfs_oper.read = dsfs::operations::read;
  // encfs_oper.write = encfs_write;
  encfs_oper.write = dsfs::operations::write;
  encfs_oper.statfs = encfs_statfs;
  encfs_oper.flush = encfs_flush;
  // encfs_oper.release = encfs_release;
  encfs_oper.release = dsfs::operations::release;
  ...
  encfs_oper.create = dsfs::operations::create;
  ...
  std::getline(cin, event_addr);

  // context is not a smart pointer because it will live for the life of
  // the filesystem.
  auto ctx = std::make_shared<dsfs::Context>(event_addr);
